...
export default function AlertsPage() {
  ...
  const { data: expiringData, isLoading: isLoadingExpiring } = useExpiringSoon(selectedBranchId, 30);
  const { data: expiredData, isLoading: isLoadingExpired } = useExpiredInventory();

  const isLoading = tab === 'low-stock' ? isLoadingLow : tab === 'expiring' ? isLoadingExpiring : isLoadingExpired;

  const rows = tab === 'low-stock' ? lowStockData
    : tab === 'expiring' ? expiringData
    : expiredData;
          ...
          <div className="flex flex-col">
            ...
            {inv.daysUntilExpiry !== undefined && (
              <span className={`text-xs font-semibold ${
                ...
                inv.daysUntilExpiry <= 7 ? 'text-amber-600' : 'text-muted-foreground'
              }`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <Tabs value={tab} onValueChange={(v) => { setTab(v as AlertTab); setCurrentPage(0); }} className="flex-1">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="low-stock" className="gap-2">
              <AlertTriangle className="h-4 w-4" />
              Low Stock
              {lowStockData?.length ? (
                <Badge variant="outline" className="h-5 px-1.5 ml-1">{lowStockData.length}</Badge>
              ) : null}
            </TabsTrigger>
            <TabsTrigger value="expiring" className="gap-2">
              ...
              Expiring
              {expiringData?.length ? (
                <Badge variant="outline" className="h-5 px-1.5 ml-1">{expiringData.length}</Badge>
              ) : null}
            </TabsTrigger>
            <TabsTrigger value="expired" className="gap-2">
              ...
              Expired
              {expiredData?.length ? (
                <Badge variant="destructive" className="h-5 px-1.5 ml-1">{expiredData.length}</Badge>
              ) : null}
            </TabsTrigger>
          </TabsList>
        </Tabs>

        <Select value={branchId} onValueChange={(val) => { setBranchId(val); setCurrentPage(0); }}>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-2">
          <p className="text-sm text-muted-foreground">
            Showing {currentPage * ITEMS_PER_PAGE + 1}-{Math.min((currentPage + 1) * ITEMS_PER_PAGE, rows?.length ?? 0)} of {rows?.length ?? 0} items
          </p>
