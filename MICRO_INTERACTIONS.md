...
<AddToCartButton productId={product.id} price={product.price} quantity={1} />;
```
...
<StockBar currentStock={15} maxStock={100} showLabel={true} />;
```
...
<PromoCodeInput onApply={(code) => handleApply(code)} />;
```
...
<CategoryPills categories={categories} activeCategory={active} onCategoryChange={setActive} />;
```
