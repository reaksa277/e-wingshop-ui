...
export function CountdownTimer({ hours, className, onExpire }: CountdownTimerProps) {
  ...
  return (
    <div className={cn('flex items-center gap-1 font-mono', className)}>
      <span className="countdown-digit">{formatNumber(timeRemaining.hours)}</span>
      <span className="text-white/60">:</span>
      <span className="countdown-digit">{formatNumber(timeRemaining.minutes)}</span>
      <span className="text-white/60">:</span>
      <span className={cn('countdown-digit', timeRemaining.seconds % 2 === 0 ? 'pulse' : '')}>
