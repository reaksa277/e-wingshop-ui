...
export const discountService = {
  ...
  getById: (id: number) => api.get<ExpiryDiscountResponse>(`/discounts/${id}`),
  ...
  getTiers: () => api.get<DiscountTierInfo[]>('/discounts/tiers'),

  create: (data: CreateDiscountRequest) => api.post<ExpiryDiscountResponse>('/discounts', data),

  autoApply: (data: AutoApplyDiscountRequest) =>
    api.post<{ tier: string; created: number; message: string }>('/discounts/auto-apply', data),

  revoke: (id: number, reason = 'Manually revoked') =>
    api.delete<ExpiryDiscountResponse>(`/discounts/${id}/revoke`, { reason }),
};
